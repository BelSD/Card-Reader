...
def ecriture(quoi):
    ...
    while continue_reading:
        ...
        if status == RFID.MI_OK:
            ...
            if lcd_lecture == False:
                mylcd.lcd_clear()
                mylcd.lcd_display_string("Ecriture Carte..", 1)
                lcd_lecture = True

            if status == RFID.MI_OK:
                #print ("Ecriture "+str(secteur)+"...")
                RFID.MFRC522_Write(id, data)
                #print (str(data)+"\n")
...
while boucle:
    ...
    if instruction == 'Creer Carte...': 
        mylcd.lcd_clear()
        BKL_LCD.ON(LCD)
        LCD_IS_ON = True
        MYSQL.Write(La_Table,"Ecriture...","instruction")
        mylcd.lcd_display_string("Attente carte...", 1)
        mylcd.lcd_display_string(time.strftime("%H:%M %d/%m/%Y"), 2)
        LED.ON(Ready_led)
        LED.ON(Read_led)
        ...
        ecriture("nr_client")
        ecriture("points")
        ecriture("date_visite")
        lcd_lecture = False
        LED.OFF(Read_led)
        LED.OFF(Ready_led)
        mylcd.lcd_clear()
        mylcd.lcd_display_string("Ecriture termine", 1)
        if buzzer == True:
            BUZZER.ON(Buzzer_1)
        sleep(1)
        BUZZER.OFF(Buzzer_1)
